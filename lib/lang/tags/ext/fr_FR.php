...
<p>Cette balise détecte la langue du navigateur, utilisée et définie par l\'utilisateur, et la compare à une liste des langues autorisées pour déterminer la langue de la session.</p>
<h3>Comment l\'utiliser ?</h3>
<p>Insérer la balise dans votre gabarit :  <em>(il peut être au-dessus de la section < head > si vous voulez)</em> et indiquez lui le nom de la langue par défaut, et les langues acceptées (seul deux caractères de noms de langue sont acceptés), puis utilisez le résultat. Exemple :</p>
...
$lang['help_function_cms_stylesheet'] = '<h3>Que fait cette balise ?</h3>
 	  ...
 	  <p>Les générations des feuilles de style sont utilisées en fonction de la date de dernière modification dans la base de données et ne sont générées que si la feuille de style a changé.</p>
...
<ul>
	...
	<li><em>(option)</em> templateid - Si templateid est défini, la balise retournera les feuilles de style associées à ce gabarit au lieu de celui en cours.</li>
...
<ul>
  ...
  <li><em>(option)</em> name=\'\' - Le nom à utiliser dans la balise img dans l\'affichage du site.</li> 
...
$lang['help_function_cms_textarea'] = '<h3>Que fait cette balise ?</h3>
  <p>Ce plugin Smarty est utilisé lors de la construction des formulaires d\'administration pour générer un champ textarea. Il dispose de plusieurs paramètres qui permettent de contrôler si un plugin WYSIWYG est utilisé <em>(si disponible)</em> ou un éditeur de syntaxe colorée, pour contrôler le comportement de ces modules, ainsi que la taille et l\'apparence du textarea.</p>
  <h3>Comment l\'utiliser ?</h3>
...
<p><em>(option)</em> autossl=1 - Activé par défaut, cette option permet de détecter si la demande faite au serveur était sur SSL, et si l\'url retournée était correctement configurée en SSL. Pour désactiver cette fonction préciser autossl = 0.</p>	
<p><em>(option)</em> assign (string) - Affecte les résultats à une variable Smarty qui porte ce nom.</p>';
...
<p>Maintenant, lorsque vous éditez une page, il y aura un texte intitulé "Second Content Block".</p></li>
	...
	<li><em>(option)</em> required (true/false) - Permet de spécifier que le bloc de contenu doit contenir du texte.</li>
...
<p>Ce plugin de fonction peut être utilisé pour fournir instructions complémentaires pour contrôler le comportement du module si différents paramètres ne peuvent pas être indiqués sur l\'URL. Ex : Dans une situation où un site est configuré pour utiliser des "pretty" URLs pour le référencement, il est souvent impossible de fournir les paramètres supplémentaires des modules, comme un detailtemplate ou ordre de tri sur une URL. Ce plugin peut être utilisé dans les gabarits de page, blocs de contenu ou d\'une manière spécifique à la page pour donner des informations complémentaires quant à la façon dont les modules devraient se comporter.</p>
<p><strong>Note :</strong> Tous les paramètres qui sont spécifiés dans l\'URL remplaceront les instructions de module. Ex : Lorsque vous utilisez Articles (News) et si un paramètre "detailtemplate" est dans l\'URL, toutes les informations antérieures du "detailtemplate" n\'auront aucun effet.</p>
...
<ul>
  ...
  <li>returnid = <em>(option integer)</em>
    <p>L\'Id de la page de contenu auquel le formulaire doit être soumis. S\'il n\'est pas spécifié, la page actuelle Id est utilisé pour les demandes du site Web (frontend). Pour les demandes d\'administration cet attribut n\'est pas nécessaire.</p>
  </li>
...
$lang['tag_adminplugin'] = 'Indique que la balise seulement est disponible dans l\'interface d\'administration qu\'elle est généralement utilisée dans les gabarits de modules.';
