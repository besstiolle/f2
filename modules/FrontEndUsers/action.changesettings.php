...
$onerow->type = ($this->GetPreference('username_is_email'))?2:0;
...
foreach( $properties as $prop ) {
  ...
  switch( $defn['type'] ) {
  case 0: // text
      $onerow->control = $this->CreateInputText( $id, $onerow->input_name, $val, $defn['length'], $defn['maxlength'] );
      break;

  case 1: // checkbox
      $onerow->control  = $this->CreateInputHidden($id, $onerow->input_name,0);
      $onerow->control  .= RRUtils::myCreateInputCheckbox( $id, $onerow->input_name, 1, $val);
      break;
  ...
  case 3: // textarea
      ...
      $onerow->wysiwyg = $flag;
      $onerow->control = $this->CreateTextArea($flag, $id, $val, $onerow->input_name);
      break;

  case 4: // dropdown
      $opts = $this->GetSelectOptions($defn['name']);
      $onerow->options = array_flip($opts);
      $onerow->control = $this->CreateInputDropdown($id, $onerow->input_name, $opts, -1, $val);
      break;
  ...
  case 5: // multiselect
      ...
      $onerow->control = $this->CreateInputSelectList($id, $onerow->input_name.'[]', $opts, $onerow->selected, $defn['length']);
      break;
  ...
  case 6: // image
      ...
      if( is_readable( $file1 ) && is_file($file1) ) {
          ...
          if( !$onerow->required ) {
              ...
              $onerow->control2 = $this->CreateInputCheckbox($id, $onerow->input_name.'_clear', 'clear','');
          }
      }
      $onerow->control = $this->CreateInputHidden($id, $onerow->input_name,$val).$this->CreateFileUploadInput($id,'feu_'.$onerow->input_name, '', $defn['length']);
      break;

  case '7': // radio group
      $opts = $this->GetSelectOptions($defn['name']);
      $onerow->options = array_flip($opts);
      $onerow->control = $this->CreateInputRadioGroup($id, $onerow->input_name, $opts, $val, '', '<br/>');
      break;
...
$thetemplate = \cge_param::get_string($params,'changesettingstemplate','feusers_changesettingsform');
$tpl = $this->CreateSmartyTemplate($thetemplate);
$tpl->assign('title',$this->Lang('user_settings')); // deprecated
$tpl->assign('startform', $this->CGCreateFormStart($id,'do_userchangesettings',$returnid));
$tpl->assign('endform', $this->CreateFormEnd());
$tpl->assign('submit', $this->CreateInputSubmit($id, 'feu_submit',$this->Lang('submit'))); // deprecated

if( isset( $params['returnto'] ) ) $tpl->assign('hidden', $this->CreateInputHidden($id,'feu_returnto',$params['returnto']));
