...
if( !$order_obj ) return;
if( $order_obj->get_status() != 'cancelled' ) return;
...
for( $s = 0; $s < $order_obj->count_destinations(); $s++ ) {
  ...
  for( $i = 0; $i < $shipping->count_all_items(); $i++ ) {
    ...
    if( $item->get_source() != 'SelfRegistration' ) continue;
    ...
    switch( $this->GetPreference('ecomm_ordercancelled') ) {
    case 'delete':
      $this->DeleteAllUserProperties('',$uid,true);
      $this->DeleteUser(Full$uid);
