...
if ($access) {
  ...
  if( isset( $_POST['add'] ) ) {
    ...
    case 'up':
      ...
      if( $handler == "" || $module == "" || $event == "" || $action == "" || ($cur_order == "" && $action != "delete") ) {
	...
	return;
      ...
      $q = 'SELECT handler_id FROM '.cms_db_prefix().'event_handlers WHERE handler_order = ? AND event_id = ?';
      $prev_id = $db->GetOne($q,array($cur_order -1,$event_id));

      if( $prev_id ) {
	$q = "UPDATE ".cms_db_prefix()."event_handlers SET handler_order = (handler_order - 1) WHERE handler_id = ?";
	...
	return;
      ...
      $q = "SELECT * FROM ".cms_db_prefix()."event_handlers WHERE handler_id = ?";
	...
	// delete a handler
	$q = "DELETE FROM ".cms_db_prefix()."event_handlers WHERE handler_id = ?";
	$dbresult = $db->Execute( $q, array( $handler ) );

	// re-order the events
	$q = "UPDATE ".cms_db_prefix()."event_handlers SET handler_order = (handler_order - 1) WHERE handler_order > ? AND event_id = ?";
